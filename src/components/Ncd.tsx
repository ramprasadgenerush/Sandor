"use client"

import React, { useEffect } from "react";
import Link from "next/link";
import Image from "next/image";

const Ncd = () => {

  useEffect(() => {
    // Re-load the main script to initialize animations and plugins on route change
    const script = document.createElement("script");
    script.src = "/assets/js/main.js";
    script.async = true;
    document.body.appendChild(script);

    return () => {
      // Cleanup the script when the component unmounts
      document.body.removeChild(script);
    };
  }, []);


  return (

    <>

      <section
        className="breadcrumb1"
        style={{ backgroundImage: "url(/assets/img/breadcrumb/breadcrumb-01.png)" }}
      >
        <div className="breadcrumb1__padding">
          <div className="container_m_30 container_l">
            <div className="row">
              <div className="col-xl-12 col_padding">
                <div className="breadcrumb1__content text-center">
                  <h3 className="breadcrumb1__title"> NCD </h3>
                  <div className="breadcrumb1__list">
                    <span>
                      <Link href="/">
                        <span>Home</span>
                      </Link>
                    </span>
                    <span>
                      <span> NCD </span>
                    </span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>


      <section className="about1 mt-50">
        <div className="content_box_pob_120">
          <div className="container_m_50 container_l">
            <div className="row">
              <div className="col-xl-10 mx-auto"> 
                <h2 className="heading1__title"> First coupon payment-<span>Notice</span></h2>
              </div>
            </div>
            <div className="row mt-5">
              <div className="col-xl-10 mx-auto">
                <div className="about1__thumb_img3 img_100">
                  <img src="assets/img/about/F-E-Scan-Copy.jpg" alt="About" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

    </>


  );

};

export default Ncd;
